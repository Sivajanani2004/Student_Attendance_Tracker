...
def signup_page():
    ...
    if password:
        ...
        st.warning(msg) if strength == "Weak" else st.success(msg)

    col1, col2 = st.columns([2, 1])
    create_clicked = col1.button("Create Account")

    if col2.button("🔐 Already have an account? Login"):
        st.session_state.page = "login"
        st.rerun()
    ...
    if create_clicked:
        ...
        res = requests.post(f"{BASE_URL}/register", json=payload, timeout=15)
...
def login_page():
    ...
    if col2.button("📝 New user? Sign up"):
    ...
    if login_clicked:
        ...
        if response.status_code == 200:
            ...
            st.session_state.role = data.get("role", "").lower()
            st.session_state.token = data.get("access_token")
            st.session_state.page = "dashboard"
            st.success("Login successful")
            st.rerun()
        else:
            st.error(safe_json(response).get("detail", "Invalid credentials"))
...
# ADMIN
    ...
    if st.session_state.role == "admin":
        menu = st.sidebar.selectbox(
            "🛠 Admin Menu",
            ["🎒 Manage Students", "🔗 Assign Students", "📅 Attendance Reports", "🗂 View Users"]
        )
        ...
        if menu == "🔗 Assign Students":
            ...
            students_map = {
                f"{s['name']} (Class {s['std_class']})": s["std_id"]
                for s in students
            }
        ...
        if menu == "📅 Attendance Reports":
            ...
            teacher_map = {
                t["user_id"]: f'{t["name"]} - ID: {t["user_id"]}'
                for t in teachers
            }
            ...
            df["student_name"] = df["student_id"].map(
                {s["std_id"]: f'{s["name"]} (ID {s["std_id"]})' for s in students}
            )
        ...
        if menu == "🗂 View Users":
            ...
            if res.status_code == 200:
                ...
                if not users:
                    ...
                    user_map = {
                        f'{u["name"]} ({u["role"]}) - ID {u["user_id"]}': u["user_id"]
                    ...
                    with col2:
                        if st.button("Remove Teacher"):
                            ...
                            res = requests.delete(
                                f"{BASE_URL}/admin/teacher/{user_id}",
                                headers=headers
                            )
                            if res.status_code == 200:
...
# TEACHER
        ...
        menu = st.sidebar.selectbox(
            "Teacher Menu",
            ["👩‍🎓 My Students", "✍️ Mark Attendance", "📅 Attendance Reports"]
        )
